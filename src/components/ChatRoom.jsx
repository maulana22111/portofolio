...
export default function ChatRoom() {
  ...
  const scrollRef = useRef(); // Ref untuk container chat
  ...
  // JURUS ANTI-LONCAT: Scroll hanya di dalam div chat
  useEffect(() => {
  ...
  return (
    <div className="bg-zinc-900/50 backdrop-blur-md border border-zinc-800 p-5 rounded-3xl shadow-2xl flex flex-col h-[500px] text-white">
      <div className="flex items-center justify-between mb-4 px-2">
        ...
        {user && <button onClick={logout} className="text-[10px] text-zinc-500 hover:text-red-400 font-bold uppercase tracking-widest transition-colors">Logout</button>}
      </div>

      {/* Container Pesan dengan Ref */}
      <div 
        ref={scrollRef}
        ...
        {user ? (
          <form onSubmit={sendMessage} className="flex gap-2 bg-zinc-800/40 p-2 rounded-2xl border border-zinc-700 focus-within:border-blue-500 transition-all">
            <input 
              ...
              autoComplete="off"
            />
            <button type="submit" className="bg-blue-600 px-5 py-2 rounded-xl text-xs font-bold hover:bg-blue-500 transition-all active:scale-95 text-white">Send</button>
          </form>
        ) : (
